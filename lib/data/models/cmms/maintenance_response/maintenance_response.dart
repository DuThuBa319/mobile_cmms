...
class MaintenanceResponse {
  ...
  MaintenanceResponseEntity getMaintenanceResponseEntity() {
    final result = MaintenanceResponseEntity(
      ...
      causeEntities: List.generate(
        cause!.length,
        (index) => cause![index].getCauseEntity(),
      ),
      code: code,
      correctionEntities: List.generate(
        correction!.length,
        (index) => correction![index].getCorrectionEntity(),
      ),
      createdAt: createdAt,
      dueDate: dueDate,
      equipmentEntity: equipment?.getEquipmentEntity(),
      estProcessTime: estProcessTime,
      id: id,
      maintenanceObject: maintenanceObject,
      materials: List.generate(
        materials!.length,
        (index) => materials![index].getMaterialEntity(),
      ),
      moldEntity: mold?.getMoldEntity(),
      note: note,
      ...
      problem: problem,
      request: request,
      responsiblePerson: responsiblePerson!.getEmployeeEntity(),
      status: status,
      updatedAt: updatedAt,
      images: images,
      sounds: sounds,
